...
| 조작 | 결과 |
|------|------|
| 배경 기포 클릭 | 터진다. 10개를 터뜨리면 기포 무리가 지나간다 |
| ↑ ↑ ↓ ↓ ← → ← → | Aero 부팅 모드 |
| → ← → ← ↓ ↓ ↑ ↑ | 숨은 화면, 무대 조명, 음원 |
| `n` `j` `z` | 커서를 따라다니는 글자 꼬리와 음원 |

판정은 `event.code` 로 한다. 한글 입력 상태에서도 그대로 통한다.
전부 Esc 로 끈다. 꼬리를 상시로 켜려면 `KonamiEgg.tsx` 에서 `{trail && ...}`
의 조건만 떼면 된다.

### 자산

이스터에그에 쓰는 그림과 음원은 제3자 저작물이다. 공개 저장소에 원본을 두면
배포 범위가 사이트 밖으로 넓어지므로 `.gitignore` 로 제외했다. 새로 받은
사본에서 되살리려면 파일을 직접 넣는다.

| 경로 | 내용 |
|------|------|
| `public/images/sans.webp` | 도트 그림. 배경이 투명해야 한다 |
| `public/audio/megalovania.mp3` | 40초로 잘라 쓴다 |
| `public/audio/newjeans.mp3` | 전곡 |
...
# 전곡을 그대로 쓰는 경우
ffmpeg -i 원본.mp3 -b:a 128k public/audio/newjeans.mp3
```

파일이 없어도 사이트는 정상 동작한다. 순서를 맞혔을 때 그 부분만 빠질 뿐이다.
